...
public class NLNoticeDao implements NoticeDao{
	...
	public void hitUp(String seq) {
		...
		// 여기까지 조회수 증가하는 작업 hit == 1
		
		// 조회수 증가 작업이 끝난 후 0.5초 동안 쉬고 있는 스레드가 조회수를 읽어감
		try {
			// 0.5 초 지난 후에 getHit_Thread 스레드가 조회수를 읽도록하기 위해서 슬립을 1초 줌
		...
		// 1초 뒤 빠져나오면 강제롤 롤백 시키기 위해 강제로 실행 예외 발생시키기
		throw new RuntimeException("> 강제로 실행 예외 발생시켜서 롤백 처리");
