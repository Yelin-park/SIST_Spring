...
public class CustomerController {
	...
	public String noticeDetail(HttpServletRequest request, HttpServletResponse response
		...
		// 스레드 2개를 생성해서 처리
		// 조회수 증가하는 스레드를 먼저 시작하도록 조회 수 읽어오는 스레드를 0.5초 슬립시킴
		new Thread(new Runnable() { // 조회수 읽어오는 스레드
			@Override
			public void run() {
				try {
					Thread.sleep(500); // 조회수를 읽어오는 일을 0.5초간 슬립
				} catch (InterruptedException e) {
					e.printStackTrace();
				} 
				noticeDao.getHit(seq); // 0.5초 후에 조회수 읽어옴
			} // run
		}, "getHit_Thread").start(); // 스레드를 생성 후 일을 시킴
		
		new Thread(new Runnable() { // 조회수 증가하는 스레드
			@Override
			public void run() {
				noticeDao.hitUp(seq); // 조회수 증가
			} // run
		}, "hitUp_Thread").start(); // 스레드를 생성 후 일을 시킴
