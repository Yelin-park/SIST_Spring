...
public class CustomerController {
	...
	*/
	// sqlSession 
	// SqlSessionTemplate 클래스를 사용하지만 부모인 SqlSession으로 주입
	@Autowired
	...
	*/
	...
	public String noticeReg(
				...
				, HttpServletRequest request
				, Principal principal
				) throws Exception{ // 커맨드 객체(업로드할 파일을 가지고 있음)
		...
		// 스프링 시큐리티 - 로그인한 id로 바꾸기
		/*
		// 1번 방법
		// 인증받은 객체 정보를 얻어와서 name을 얻어오면 됨
		// getPrincipal() : 인증받은 객체의 정보를 돌려주는 함수
		UserDetails user = (UserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
		notice.setWriter(user.getUsername()); // 로그인 인증받은 id로 바꿈!
		*/
		
		// 2번 방법 Principal 파라미터를 받아서 사용
	...
	public String notices(HttpServletRequest request, HttpServletResponse response, Model model
		...
		// root-context에 설정한 여러개의 Mapper중 내가 원하는 것만 가져오도록 해당 인터페이스를 넣어줌
		NoticeDao mybatisNoticeDao = this.sqlSession.getMapper(NoticeDao.class);
		List<Notice> list = mybatisNoticeDao.getNotices(ppage, pfield, pquery);
